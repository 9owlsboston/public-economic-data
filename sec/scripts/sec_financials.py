...
Reads per-company JSON files from sec/financials/{tpid}.json.
...
class SECFinancials:
    ...
    def get(self, tpid: int) -> dict | None:
        """Load full financial data for a company by TPID.

        Returns the parsed JSON or None if not found.
        """
        if tpid in self._cache:
            return self._cache[tpid]

        data = None
        if self._mode == "local":
            path = self._dir / f"{tpid}.json"
            if path.exists():
                with open(path) as f:
                    data = json.load(f)
        elif self._mode == "github":
            url = f"{self._github_url}/{tpid}.json"
        ...
        if data:
            self._cache[tpid] = data
        return data

    def latest_annual(self, tpid: int) -> dict | None:
    ...
    def latest_quarter(self, tpid: int) -> dict | None:
    ...
    def yoy_revenue_growth(self, tpid: int) -> float | None:
    ...
    def revenue_trend(self, tpid: int, periods: int = 5) -> list[dict]:
